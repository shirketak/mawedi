...
@section('page-title', 'أكثر المستشفيات حجزاً')
...
<div class="card">
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            <thead><tr><th>#</th><th>المستشفى</th><th>عدد الحجوزات</th></tr></thead>
            <tbody>
                @forelse($items as $index => $item)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $item['name'] }}</td>
                    <td><span class="badge bg-primary">{{ number_format($item['total']) }}</span></td>
                </tr>
                @empty
